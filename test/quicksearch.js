...
describe('quick search--', function() {
    ...
    describe('quick search factory--', function () {
        ...
        it('must return FlexData quick search processor for input search', function () {
            expect(ogrid.QSearchFactory.parse('weather 60601'))
                .to.be.an.instanceof(ogrid.QSearchProcessor.FlexData);
        });
        it('must return FlexData quick search processor for input search', function () {
    ...
    //weather does not require any filter now due to switch to FlexData
    var weatherTests = _getSearchTests('weather 60601', weather60601MockData);
    describe('weather quick search processor', function () {
        weatherTests();
    });

    describe('flex search builder', function () {
        var columns = ['screenName', 'text'];
        var fb = new ogrid.FlexSearchBuilder();
        ...
        it('must return true if input is valid flex query and trigger word is valid', function () {
            expect(fb.matchesInput('tweet "filter"', ['weather','tweet'])).to.equal(true);
            expect(fb.matchesInput('weather "filter"', ['weather','tweet'])).to.equal(true);
        });
        it('must return false if input is a valid flex query but trigger word is invalid', function () {
            expect(fb.matchesInput('buzz "lighyear"', ['weather','tweet'])).to.equal(false);
        });
        it('must return correct simple filter', function () {
            var filter = "coupon";
            var f = JSON.stringify({"$or":[ {screenName:{"$regex":filter,"$options":"i"}}, {text:{"$regex":filter,"$options":"i"}}]});
            expect(JSON.stringify(fb.parse('tweet ' + filter).getFilter(columns))).to.equal(f);
        });
        ...
        it('must return correct max with simple filter and max specified', function () {
            expect(fb.parse('tweet "filter" $max:5 $oo:v').getLimit()).to.equal(5);
        });
        ...
        it('must return 0 when max not specified', function () {
            expect(fb.parse('tweet "filter"').getLimit()).to.equal(0);
        });
        it('must return expected filter based on multiple params', function () {
            var f = JSON.stringify({"$and":[ {screenName:{"$regex":"v1","$options":"i"}}, {text:{"$regex":"v 2","$options":"i"}}]});
    ...
    describe('flex data quick search processor', function () {
        var fd = ogrid.QSearchProcessor.flexData();
        fd.setOptions({
            datasets: ogrid.Mock.data.datasets
        })

        it('must return true if input is valid flex query and trigger word is valid', function () {
            expect(fd.test('tweet "filter"')).to.equal(true);
            expect(fd.test('weather "filter"')).to.equal(true);
        });
        it('must return true if input is valid flex query and trigger aliases are used', function () {
            expect(fd.test('twe "filter"')).to.equal(true);
            expect(fd.test('wea "filter"')).to.equal(true);
        });
        it('must return false if input is a valid flex query but trigger word is invalid', function () {
            expect(fd.test('buzz "lighyear"')).to.equal(false);
        });
        it('must return correct dataset given trigger word or alias', function () {
            expect(fd._getDatasetByTrigger('tweet').id).to.equal('twitter');
            expect(fd._getDatasetByTrigger('twe').id).to.equal('twitter');
        });
        it('must resolve column names correctly when using column aliases and dataset uses qualified column names', function () {
            fd.getFlexSearchBuilder().parse('license legal_name:v1 lic_status:v2');
            var ds = fd._getDatasetByTrigger(fd.getFlexSearchBuilder().getTrigger());
            var params = fd.getFlexSearchBuilder().getParams();
            fd._resolveColumnAliases(ds, params);
            var f = JSON.stringify({"$and":[ {"what.legal_name":{"$regex":"v1","$options":"i"}}, {"what.lic_status":{"$regex":"v2","$options":"i"}}]});
            expect(JSON.stringify(fd.getFlexSearchBuilder().getFilterFromParams(params))).to.equal(f);
        });

        it('must resolve column names correctly when not using column aliases', function () {
            fd.getFlexSearchBuilder().parse('tweet screenName:v1 text:v2');
            var ds = fd._getDatasetByTrigger(fd.getFlexSearchBuilder().getTrigger());
            var params = fd.getFlexSearchBuilder().getParams();
            fd._resolveColumnAliases(ds, params);
            var f = JSON.stringify({"$and":[ {"screenName":{"$regex":"v1","$options":"i"}}, {"text":{"$regex":"v2","$options":"i"}}]});
            expect(JSON.stringify(fd.getFlexSearchBuilder().getFilterFromParams(params))).to.equal(f);
        });
    });
