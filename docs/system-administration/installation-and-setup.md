...
#### User Interface
* Configure various application settings as required by your application by editing `src/js/custom/Config.js`. The settings include session timeout, area to limit address quick searches, to name a few. See [OpenGrid Configuration](../system-administration/opengrid-configuration/) for more details
...
After a successful build as documented on the [User Interface Build Procedures](../system-administration/build-procedures/), a `/dist` folder is created with the files necessary for deployment to a web server. Copy all the files from the `/dist` folder, including sub-folders, to your application's root folder on the web server. 
