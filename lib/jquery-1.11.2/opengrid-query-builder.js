...
// ===============================
    ...
    QueryBuilder.prototype.createRuleInput = function(rule) {
        ...
        for (var i=0; i<rule.operator.nb_inputs; i++) {
            ...
            //create corresponding auto-complete object
            if (filter.autocomplete) {
                $ruleInput.autocomplete(filter.autocomplete);
            }
        ...
        //WindyGrid2 - add custom dp.change event emitted by new datetime picker
        $inputs.on('change dp.change', function() {
...
<li id="'+ rule_id +'" class="rule-container"> \
  ...
  <div class="rule-value-container"></div> \
    ...
    QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
        ...
        filter.placeholder = "ENTER FILTER VALUE";
            ...
            return (function parse(data) {
                ...
                data.rules.forEach(function(rule) {
                    if (rule.rules && rule.rules.length>0) {
                        ...
                        //use copy of values array, to get past validation error when using between on dates
                        var valuesCopy = [];
                        if (ope.nb_inputs !== 0) {
                            if (!(rule.value instanceof Array)) {
                                rule.value = that._getRuleValue(rule);
                                valuesCopy = [rule.value];
                            } else {
